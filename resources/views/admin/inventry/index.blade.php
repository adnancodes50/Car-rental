                    ...
                    <tbody>
                        @foreach ($addOns as $addOn)
                            <tr>
                                ...
                                {{-- <td>{{ $addOn->total_booked }}</td>  <!-- removed --> --}}
                                ...
                                <td class="text-center">
                                    <div class="d-flex justify-content-center gap-2">
                                        <a href="{{ route('inventry.view', $addOn->id) }}"
...
<style>
    table.table-hover tbody tr:hover { background-color: rgba(255, 193, 7, 0.1); transition: background-color .2s; }
    .btn-outline-info:hover { background-color:#e7f9fc!important; color:#0dcaf0!important; }
    .btn-outline-info:hover i { color:#0dcaf0!important; }
    .action-btn { display:flex; align-items:center; justify-content:center; width:32px; height:32px; border-radius:6px; padding:0; }
...
$(function () {
    $('#addonsTable').DataTable({
        ...
        columnDefs: [
            { orderable: false, targets: [7] },      // Actions (moved from 8 -> 7)
            { searchable: false, targets: [7] },     // Actions
            { targets: 0, responsivePriority: 1 },   // Image
            { targets: 7, responsivePriority: 2 }    // Actions
        ],
        language: { emptyTable: "No add-ons found." }
    });
