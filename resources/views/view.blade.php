...
document.addEventListener("DOMContentLoaded", function () {
 ...
 function toMidnight(val) {
  ...
  const d = new Date(val.getFullYear(), val.getMonth(), val.getDate(), 0, 0, 0, 0);
  return d;
}
...
function addMonthsInclusive(start, months, extraDays) {
  // Move to same day N months later (engine-safe), then step back 1 day (inclusive), then add extras
  const exclusive = new Date(start.getFullYear(), start.getMonth() + months, start.getDate(), 0, 0, 0, 0);
  let end = addDays(exclusive, -1 + (extraDays || 0));
  return toMidnight(end);
}
...
function fmt(d) {
  ...
  // Expect each item like {from:'YYYY-MM-DD', to:'YYYY-MM-DD'} (inclusive)
 // Expect [{from:'YYYY-MM-DD', to:'YYYY-MM-DD'}, ...]
...
function hasOverlap(a1, a2) {
  return bookedRanges.some(({from, to}) => a1 <= to && a2 >= from);
}
...
const flatpickrInstance = flatpickr("#rentalStartDate", {
  ...
  function calculateTotal() {
    ...
    if (selectedType === "week")  end = addDays(start, qty * 7 + extraDays - 1);
    if (selectedType === "month") {
      const tmp = new Date(start);
      tmp.setMonth(tmp.getMonth() + qty);      // exclusive
      end = addDays(tmp, -1 + extraDays);      // make inclusive + extras
      end = toMidnight(end);
    }

    // Hard stop if overlaps
    if (hasOverlap(start, end)) {
      ...
      Swal.fire({
        ...
        html: `<p>Your selected period overlaps with an existing booking.</p>
    ...
    // Show extra-days selector (for week/month)
    ...
    if (extraDays > 0) {
      const perExtra = (selectedType === "week")
        ? (selectedPrice / 7)
        : (selectedType === "month" ? (selectedPrice / 30) : selectedPrice);
  ...
  optionCards.forEach(card => {
    card.addEventListener("click", () => {
      ...
      qtyLabel.className = "form-label mb-1";
      qtyLabel.textContent =
        selectedType === "day" ? "Number of Days" :
      ...
      if (selectedType !== "day") {
        ...
        flex.appendChild(extraDiv);
      }

      quantitySection.appendChild(flex);
  ...
  document.addEventListener('change', function (e) {
    if (e.target && e.target.name === 'booking_payment_method') {
      ...
      bootstrap.Modal.getInstance(document.getElementById("bookingPayment"))?.hide();

      if (method === 'stripe') {
        new bootstrap.Modal(document.getElementById("bookingStripeModal")).show();
      } else if (method === 'payfast') {
        // TODO: Implement PayFast redirect here
