...
<div class="container py-4 py-lg-5 mt-5">
    ...
    <div class="row g-4 g-lg-5 align-items-stretch">
        ...
        <div class="col-12 col-md-6 d-flex">
            <div class="card shadow-lg border-3 rounded-4 flex-fill equipment-card"
                style="border-color: #679767;">
                <div class="ratio ratio-4x3 equipment-image-wrapper">
        ...
        <div class="col-12 col-md-6 d-flex">
            <div class="card border-0 shadow-sm p-4 rounded-4 flex-fill equipment-card d-flex flex-column">
                <div class="d-flex justify-content-between align-items-center mb-2 flex-wrap">
                    <h2 class="h4 fw-bold mb-0">{{ $equipment->name }}</h2>
                    <h2 class="text-muted small mb-0">
                        Category:
                        <span class="fw-semibold text-dark">{{ $equipment->category->name ?? 'N/A' }}</span>
                    </h2>
                </div>
                ...
                @php
                    ...
                    $hasLongDescription = strlen($plainDescription) > 160;
                ...
                <div class="d-flex flex-wrap gap-3 mt-1">
                    ...
                    @if ($equipment->category && $equipment->category->is_for_sale)
                        <a href="javascript:void(0)"
                            class="btn flex-grow-1 py-3 fw-semibold purchase-trigger
                            {{ ($equipment->status ?? null) === 'sold' ? 'disabled' : '' }}"
                            data-bs-toggle="modal" data-bs-target="#purchaseModal"
                            style="background-color:white;color:black;border:2px solid #679767;transition:all .3s;
                                {{ ($equipment->status ?? null) === 'sold' ? 'pointer-events:none;opacity:.55;' : '' }}"
                        ...
                        <div class="row row-cols-1 row-cols-sm-2 g-2 text-muted small">
                            @foreach ($equipment->stocks as $stock)
                                <div class="col d-flex align-items-center">
                                    ...
                                    <span>{{ $stock->location->name ?? 'Unknown Location' }} — {{ $stock->stock }} in stock</span>
                                </div>
                            @endforeach
                        </div>
                    </div>
...
<style>
    /* Equal height cards for both columns */
    .equipment-card {
        height: 100%;
        min-height: 450px;
        display: flex;
